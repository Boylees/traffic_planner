...
/**
 * @brief 以JSON格式打印规划好的路径。
 * @param network 交通网络对象。
 * @param path 要打印的路径。
 */
static void print_route_json(const TrafficNetwork* network, const RoutePath* path) {
    if (!path) {
...
int main() {
    ...
    while (1) {
        ...
        switch (choice) {
            case 1: handle_single_path_planning(network); break;
            case 2: handle_tsp_planning(network); break;
            case 3: handle_sequential_planning(network); break;
            case 4: printf("感谢使用！\n"); goto end; // 跳转到清理步骤
            default: printf("无效输入，请输入1-4之间的数字。\n");
        }
    }
